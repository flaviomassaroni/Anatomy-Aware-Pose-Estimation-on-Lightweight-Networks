...
from config import STL_BETA, SEVERITY_ALPHA
...
def test_severity_weighting():
    """Verifica che SEVERITY_ALPHA=2.0 produca loss > SEVERITY_ALPHA=0 su posa violante."""
    print("\n=== Test severity weighting ===")

    B, K = 2, 17
    coords = torch.zeros(B, K, 2, dtype=torch.float64)
    valid_mask = torch.ones(B, K, dtype=torch.float64)
    # Braccio sx ~3x piu' lungo del dx: forte violazione simmetria
    coords[:, 5]  = torch.tensor([20.0, 10.0])  # left_shoulder
    coords[:, 6]  = torch.tensor([30.0, 10.0])  # right_shoulder
    coords[:, 7]  = torch.tensor([20.0, 70.0])  # left_elbow (lungo)
    coords[:, 8]  = torch.tensor([30.0, 30.0])  # right_elbow (corto)
    coords[:, 9]  = torch.tensor([20.0, 90.0])  # left_wrist
    coords[:, 10] = torch.tensor([30.0, 50.0])  # right_wrist
    coords[:, 11] = torch.tensor([22.0, 30.0])
    coords[:, 12] = torch.tensor([28.0, 30.0])
    coords[:, 13] = torch.tensor([22.0, 55.0])
    coords[:, 14] = torch.tensor([28.0, 55.0])
    coords[:, 15] = torch.tensor([22.0, 80.0])
    coords[:, 16] = torch.tensor([28.0, 80.0])

    orig_alpha = stl_module.SEVERITY_ALPHA
    stl_module.SEVERITY_ALPHA = 0.0
    loss_alpha0 = bone_ratio_loss(coords, valid_mask)
    stl_module.SEVERITY_ALPHA = 2.0
    loss_alpha2 = bone_ratio_loss(coords, valid_mask)
    stl_module.SEVERITY_ALPHA = orig_alpha

    print(f"  Loss SEVERITY_ALPHA=0.0: {loss_alpha0.item():.6f}")
    print(f"  Loss SEVERITY_ALPHA=2.0: {loss_alpha2.item():.6f} (atteso: maggiore)")
    ok = float(loss_alpha2) > float(loss_alpha0)
    assert ok, "Severity weighting inattivo! alpha=2.0 deve dare loss > alpha=0.0"
    print("  Severity weighting attivo: OK")
    return ok
...
if __name__ == '__main__':
    ...
    results.append(('severity_weighting',  test_severity_weighting()))
